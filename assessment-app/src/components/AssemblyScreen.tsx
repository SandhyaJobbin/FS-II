    ...
    <div className="w-full max-w-[500px] mx-auto animate-fade-in">
      <div className="bg-card backdrop-blur-md border border-[var(--card-border)] rounded-2xl p-8 md:p-10 shadow-2xl flex flex-col gap-6 relative overflow-hidden before:content-[''] before:absolute before:top-0 before:left-0 before:right-0 before:height-[3px] before:bg-linear-to-r before:from-[#4facfe] before:to-[#00f2fe]">
        ...
        <div className="text-center">
          ...
          <h2 className="text-2xl font-bold tracking-tight mb-1">Attempt Registered!</h2>
          <p className="text-xs text-[var(--text-secondary)]">
            Your secure question bank has been assembled and frozen on the server.
          </p>
        </div>

        <div className="bg-slate-950/40 border border-[var(--card-border)] rounded-lg p-4 flex flex-col gap-2.5 text-sm">
          ...
          <div className="flex justify-between">
            ...
            <span className="font-mono font-semibold text-white text-xs">{attemptId}</span>
          </div>
        ...
        <div className="border border-[var(--card-border)] rounded-lg p-4 bg-slate-900/10 flex flex-col gap-3">
        ...
        <div className="flex flex-col gap-3">
          ...
          <ul className="flex flex-col gap-1.5 max-h-[120px] overflow-y-auto pr-1">
            {Object.entries(breakdown).map(([label, count]) => (
              <li key={label} className="text-[11px] bg-slate-900/20 border border-[var(--card-border)] px-3 py-2 rounded-lg flex justify-between">
        ...
        <div className="flex gap-3">
          <button
            type="button"
            onClick={onReset}
            className="flex-1 font-semibold text-[13px] py-3 rounded-lg bg-slate-900/60 border border-[var(--card-border)] text-white hover:bg-slate-800/80 cursor-pointer transition-all"
          >
            Reset Session
          </button>
          <button
